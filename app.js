...
async function applyPublicContent() {
  ...
  try {
    ...
    const serviceTypes = ['umrah_fare','umrah_package','holiday_package','medical_tourism','card_offer','airline_offer','explore']; const serviceItems = items.filter(item => serviceTypes.includes(item.type)); const serviceGrid = $('#serviceContentGrid'); if (serviceGrid) serviceGrid.innerHTML = serviceItems.length ? serviceItems.map(item => `<article class="service-content-card">${item.imageUrl ? `<img src="${escapeHtml(item.imageUrl)}" alt="${escapeHtml(item.title)}" loading="lazy" />` : '<div class="service-content-placeholder">Sadik Travels</div>'}<div class="service-content-card-body"><small>${escapeHtml(String(item.type).replace(/_/g,' '))}</small><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.subtitle || item.description || '')}</p>${item.metadata?.price ? `<strong>৳${escapeHtml(Number(item.metadata.price).toLocaleString('en-BD'))}</strong>` : ''}</div></article>`).join('') : empty('No featured services yet', 'Published offers and service content will appear here.');
