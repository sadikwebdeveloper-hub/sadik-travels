...
export class SQLiteStore implements Store {
  ...
  async updateAdminBooking(id: string, patch: { status?: BookingStatus; internalNote?: string; ownerId?: string | null }, actorId: string, action = 'updated') {
    const current = await this.findBooking(id); if (!current) return undefined;
    const updated = { ...current, ...(patch.status === undefined ? {} : { status: patch.status }), ...(patch.internalNote === undefined ? {} : { internalNote: patch.internalNote }), ...(patch.ownerId === undefined ? {} : { ownerId: patch.ownerId ?? undefined }), updatedAt: now() };
    this.db.prepare('UPDATE bookings SET status=?,provider_ref=?,request=?,response=?,owner_id=?,internal_note=?,updated_at=? WHERE id=?').run(updated.status,updated.providerRef ?? null,json(updated.request),updated.response === undefined ? null : json(updated.response),updated.ownerId ?? null,updated.internalNote ?? null,updated.updatedAt,id);
    if (current.status !== updated.status || current.internalNote !== updated.internalNote || current.ownerId !== updated.ownerId) await this.addBookingEvent({ bookingId: id, actorId, action, fromStatus: current.status, toStatus: updated.status, note: updated.internalNote });
  ...
  async adminStats(): Promise<AdminStats> {
    ...
    return { bookings: { total: Object.values(counts).reduce((sum, value) => sum + value, 0), ...counts }, revenueBdt: revenue, customers, tours: tourRows, supportTickets: ticketCounts, statusDistribution: statuses.map(status => ({ status, count: counts[status] })).filter(item => item.count > 0) };
  }
