...
import { optionalAuth, requireAuth, requireAdmin, requirePermission, permissionsFor, notFound, requestContext } from './middleware.js';
...
const ADMIN_ROLES = ['admin', 'manager', 'super_admin', 'support', 'content_manager', 'finance'] as const;
...
export function buildApp() {
  ...
  app.get('/api/v1/admin/me', requireAdmin(store), (req, res) => res.json({ user: req.user, permissions: permissionsFor(req.user).map(permission => permission.replace(':', '_')) }));
  app.get('/api/v1/admin/stats', requirePermission(store, 'dashboard:view'), async (_req, res) => { const stats = await store.adminStats(); const recent = await store.listAdminBookings({ page: 1, pageSize: 8 }); res.json({ ...stats, tours: stats.tours, recentBookings: recent.bookings, recentActivity: await store.listAuditLogs(12) }); });
  app.get('/api/v1/admin/activity', requirePermission(store, 'audit:view'), async (_req, res) => res.json({ activity: await store.listAuditLogs(50) }));
  app.get('/api/v1/admin/audit-logs', requirePermission(store, 'audit:view'), async (req, res) => res.json(await store.listAuditLogsDetailed({ q: req.query.q ? String(req.query.q) : undefined, action: req.query.action ? String(req.query.action) : undefined, page: Number(req.query.page) || 1, pageSize: Number(req.query.pageSize) || 30 })));
  app.get('/api/v1/admin/users', requirePermission(store, 'users:manage'), async (_req, res) => res.json({ users: await store.listUsers() }));
  app.patch('/api/v1/admin/users/:id/role', requirePermission(store, 'users:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(roleRequest, req.body); const target = await store.findUserById(String(req.params.id)); assert(target, 404, 'USER_NOT_FOUND', 'User not found'); assert(target.id !== req.user!.id || input.role === 'super_admin', 409, 'SELF_ROLE_CHANGE_BLOCKED', 'You cannot remove your own admin access'); assert(req.user!.role === 'super_admin' || input.role !== 'super_admin', 403, 'SUPER_ADMIN_REQUIRED', 'Only a super admin can grant super-admin access'); const user = await store.setUserRole(target.id, input.role); await store.audit('admin.user_role_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { targetUserId: target.id, previousRole: target.role, role: input.role } }); res.json({ user }); });

  app.get('/api/v1/admin/services', requirePermission(store, 'services:manage'), async (_req, res) => res.json({ services: await store.getServiceVisibility() }));
  app.patch('/api/v1/admin/services/:key/status', requirePermission(store, 'services:manage'), async (req, res) => { const key = String(req.params.key) as any; const input = toInput(z.object({ status: serviceStatusSchema, confirm: z.boolean().default(false) }), req.body); const current = (await store.getServiceVisibility()).find(item => item.key === key); assert(current, 404, 'SERVICE_NOT_FOUND', 'Service not found'); assert(input.status === current.status || input.confirm, 400, 'CONFIRMATION_REQUIRED', `Confirm changing ${current.label} visibility`); const service = await store.updateServiceVisibility(key, input.status, req.user!.id); await store.audit('admin.service_visibility_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { service: key, previousStatus: current.status, status: input.status } }); res.json({ service }); });

  app.get('/api/v1/admin/customers', requirePermission(store, 'customers:view'), async (req, res) => res.json(await store.listAdminCustomers({ q: req.query.q ? String(req.query.q) : undefined, status: req.query.status === 'active' || req.query.status === 'blocked' || req.query.status === 'pending' ? req.query.status : 'all', page: Number(req.query.page) || 1, pageSize: Number(req.query.pageSize) || 20 })));
  app.get('/api/v1/admin/customers/:id', requirePermission(store, 'customers:view'), async (req, res) => { const customer = await store.findAdminCustomer(String(req.params.id)); assert(customer, 404, 'CUSTOMER_NOT_FOUND', 'Customer not found'); res.json({ customer }); });
  ...
  app.get('/api/v1/admin/settings', requirePermission(store, 'settings:manage'), async (_req, res) => { const saved = new Map((await store.getAdminSettings()).map(item => [item.key, item])); const settings = await Promise.all(SETTING_KEYS.map(async key => { const item = saved.get(key); if (item) return item; const value = await store.getSetting(key); return { key, configured: Boolean(value), secret: SETTING_SECRET_KEYS.has(key), ...(SETTING_SECRET_KEYS.has(key) ? { masked: value ? SECRET_MASK : '' } : { value: value ?? '' }) }; })); res.json({ settings }); });
  app.put('/api/v1/admin/settings', requirePermission(store, 'settings:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(settingPatchSchema, req.body) as Record<string, string | undefined>; const patch = Object.fromEntries(Object.entries(input).filter(([key, value]) => value !== undefined && !(SETTING_SECRET_KEYS.has(key) && value === SECRET_MASK)).map(([key, value]) => [key, typeof value === 'boolean' ? String(value) : value])); await store.updateSettings(patch, req.user!.id); await store.audit('admin.settings_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { keys: Object.keys(patch) } }); res.json({ settings: await store.getAdminSettings() }); });
  app.post('/api/v1/admin/settings/test-sms', requirePermission(store, 'settings:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(messageTestRequest, req.body); const result = await messaging.sendSms(input.destination, input.message); res.json({ sent: true, result }); });
  app.post('/api/v1/admin/settings/test-email', requirePermission(store, 'settings:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(messageTestRequest, req.body); const result = await messaging.sendEmail(input.destination, input.subject || 'Sadik Travels test email', input.message); res.json({ sent: true, result }); });

  // Booking assignment and lifecycle controls. Claiming is atomic at the database layer.
  app.get('/api/v1/admin/bookings', requirePermission(store, 'bookings:view'), async (req, res) => { const rawStatus = String(req.query.status || ''); const status = rawStatus === 'needs_review' ? 'needs_review' : (bookingStatusSchema.safeParse(rawStatus).success ? rawStatus as BookingStatus : 'all'); const vertical = verticalSchema.safeParse(String(req.query.vertical || '')); const result = await store.listAdminBookings({ q: req.query.q ? String(req.query.q) : undefined, status, vertical: vertical.success ? vertical.data : 'all', ownerId: req.query.ownerId ? String(req.query.ownerId) : undefined, page: Number(req.query.page) || 1, pageSize: Number(req.query.pageSize) || 20 }); res.json(result); });
  app.get('/api/v1/admin/bookings/:id', requirePermission(store, 'bookings:view'), async (req, res) => { const result = await store.listAdminBookings({ q: String(req.params.id), page: 1, pageSize: 1 }); const booking = result.bookings[0]; assert(booking, 404, 'BOOKING_NOT_FOUND', 'Booking not found'); res.json({ booking, history: await store.listBookingEvents(booking.id) }); });
  app.post('/api/v1/admin/bookings/:id/claim', requirePermission(store, 'bookings:manage'), async (req, res) => { const existing = await store.findBooking(String(req.params.id)); assert(existing, 404, 'BOOKING_NOT_FOUND', 'Booking not found'); if (existing.ownerId && existing.ownerId !== req.user!.id) throw new AppError(409, 'BOOKING_ALREADY_CLAIMED', 'This booking is already assigned to another operator'); const booking = await store.claimBooking(existing.id, req.user!.id); assert(booking, 409, 'BOOKING_ALREADY_CLAIMED', 'This booking is already assigned to another operator'); await store.audit('admin.booking_claimed', { ...clientMeta(req), userId: req.user!.id, metadata: { bookingId: existing.id } }); res.json({ booking }); });
  app.post('/api/v1/admin/bookings/:id/release', requirePermission(store, 'bookings:manage'), async (req, res) => { const existing = await store.findBooking(String(req.params.id)); assert(existing, 404, 'BOOKING_NOT_FOUND', 'Booking not found'); const booking = await store.releaseBooking(existing.id, req.user!.id, isPrivileged(req)); assert(booking, 403, 'BOOKING_OWNER_REQUIRED', 'Only the assigned operator can release this booking'); await store.audit('admin.booking_released', { ...clientMeta(req), userId: req.user!.id, metadata: { bookingId: existing.id } }); res.json({ booking }); });
  app.patch('/api/v1/admin/bookings/:id', requirePermission(store, 'bookings:manage'), async (req, res) => { const input = toInput(adminBookingPatchRequest, req.body); const existing = await store.findBooking(String(req.params.id)); assert(existing, 404, 'BOOKING_NOT_FOUND', 'Booking not found'); assert(isPrivileged(req) || existing.ownerId === req.user!.id, 403, 'BOOKING_OWNER_REQUIRED', 'Claim this booking before changing it'); if (input.ownerId !== undefined) { assertPrivileged(req); if (input.ownerId) { const assignee = await store.findUserById(input.ownerId); assert(assignee && ['manager', 'admin', 'super_admin', 'support', 'content_manager', 'finance'].includes(assignee.role) && assignee.status === 'active', 400, 'INVALID_ASSIGNEE', 'Choose an active admin or operator'); } } if (input.status && input.status !== existing.status) { assert(bookingTransitions[existing.status].includes(input.status), 409, 'INVALID_BOOKING_TRANSITION', `A ${existing.status} booking cannot move directly to ${input.status}`); } const action = input.ownerId !== undefined ? 'assignee_changed' : input.request !== undefined ? 'request_updated' : input.status ? 'status_changed' : 'note_added'; const booking = await store.updateAdminBooking(existing.id, { status: input.status, internalNote: input.internalNote, ownerId: input.ownerId, request: input.request }, req.user!.id, action); assert(booking, 404, 'BOOKING_NOT_FOUND', 'Booking not found'); await store.audit('admin.booking_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { bookingId: existing.id, previousStatus: existing.status, status: input.status, previousOwnerId: existing.ownerId, ownerId: input.ownerId } }); res.json({ booking, history: await store.listBookingEvents(existing.id) }); });

  app.get('/api/v1/admin/tickets', requirePermission(store, 'support:manage'), async (req, res) => res.json({ tickets: await store.listSupportTickets({ status: ['open','pending','in_progress','waiting_customer','resolved','closed'].includes(String(req.query.status)) ? String(req.query.status) as any : 'all', q: req.query.q ? String(req.query.q) : undefined }) }));
  app.get('/api/v1/admin/tickets/:id', requirePermission(store, 'support:manage'), async (req, res) => { const ticket = await store.findSupportTicket(String(req.params.id)); assert(ticket, 404, 'TICKET_NOT_FOUND', 'Support ticket not found'); res.json({ ticket, messages: await store.listSupportMessages(ticket.id) }); });
  app.patch('/api/v1/admin/tickets/:id', requirePermission(store, 'support:manage'), async (req, res) => { const input = toInput(supportPatchRequest, req.body); if (input.assignedTo) { const assignee = await store.findUserById(input.assignedTo); assert(assignee && ['manager','admin','super_admin','support'].includes(assignee.role) && assignee.status === 'active', 400, 'INVALID_ASSIGNEE', 'Choose an active support operator'); } const ticket = await store.updateSupportTicket(String(req.params.id), input); assert(ticket, 404, 'TICKET_NOT_FOUND', 'Support ticket not found'); await store.audit('admin.ticket_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { ticketId: ticket.id, status: ticket.status, priority: ticket.priority, assignedTo: ticket.assignedTo } }); res.json({ ticket }); });
  app.post('/api/v1/admin/tickets/:id/messages', requirePermission(store, 'support:manage'), async (req, res) => { const input = toInput(supportMessageRequest, req.body); const ticket = await store.findSupportTicket(String(req.params.id)); assert(ticket, 404, 'TICKET_NOT_FOUND', 'Support ticket not found'); const message = await store.createSupportMessage({ ticketId: ticket.id, authorId: req.user!.id, authorType: 'admin', message: input.message, internal: input.internal }); await store.audit('admin.ticket_message_added', { ...clientMeta(req), userId: req.user!.id, metadata: { ticketId: ticket.id, internal: input.internal } }); res.status(201).json({ message }); });

  app.get('/api/v1/admin/content', requirePermission(store, 'content:manage'), async (req, res) => { const type = contentTypeSchema.safeParse(String(req.query.type || '')); const status = contentStatusSchema.safeParse(String(req.query.status || '')); res.json({ content: await store.listContent({ type: type.success ? type.data : 'all', status: status.success ? status.data : 'all', q: req.query.q ? String(req.query.q) : undefined, includeArchived: true }) }); });
  app.post('/api/v1/admin/content', requirePermission(store, 'content:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(contentInputSchema, req.body); const item = await store.createContent({ ...input, createdBy: req.user!.id }); await store.audit('admin.content_created', { ...clientMeta(req), userId: req.user!.id, metadata: { contentId: item.id, type: item.type } }); res.status(201).json({ content: item }); });
  app.patch('/api/v1/admin/content/:id', requirePermission(store, 'content:manage'), async (req, res) => { assertPrivileged(req); const input = toInput(contentPatchSchema, req.body); const item = await store.updateContent(String(req.params.id), input); assert(item, 404, 'CONTENT_NOT_FOUND', 'Content item not found'); await store.audit('admin.content_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { contentId: item.id, type: item.type } }); res.json({ content: item }); });
  app.delete('/api/v1/admin/content/:id', requirePermission(store, 'content:manage'), async (req, res) => { assertPrivileged(req); const item = await store.archiveContent(String(req.params.id)); assert(item, 404, 'CONTENT_NOT_FOUND', 'Content item not found'); await store.audit('admin.content_archived', { ...clientMeta(req), userId: req.user!.id, metadata: { contentId: item.id } }); res.json({ content: item }); });

  app.get('/api/v1/admin/tours', requirePermission(store, 'content:manage'), async (req, res) => { const filters: TourFilters = { q: req.query.q ? String(req.query.q) : undefined, country: req.query.country ? String(req.query.country) : undefined, tourType: req.query.tourType ? String(req.query.tourType) : undefined, status: req.query.status === 'draft' || req.query.status === 'published' || req.query.status === 'archived' ? req.query.status : undefined, sort: req.query.sort === 'price_asc' || req.query.sort === 'price_desc' ? req.query.sort : 'newest' }; res.json({ tours: await store.listTours(filters) }); });
  app.post('/api/v1/admin/tours', requirePermission(store, 'content:manage'), async (req, res) => { const input = toInput(tourInputSchema, req.body) as CreateTour; const tour = await store.createTour({ ...input, createdBy: req.user!.id }); await store.audit('admin.tour_created', { ...clientMeta(req), userId: req.user!.id, metadata: { tourId: tour.id } }); res.status(201).json({ tour }); });
  app.patch('/api/v1/admin/tours/:id', requirePermission(store, 'content:manage'), async (req, res) => { const input = toInput(tourPatchSchema, req.body) as UpdateTour; const tour = await store.updateTour(String(req.params.id), input); assert(tour, 404, 'TOUR_NOT_FOUND', 'Tour package not found'); await store.audit('admin.tour_updated', { ...clientMeta(req), userId: req.user!.id, metadata: { tourId: tour.id } }); res.json({ tour }); });
  app.delete('/api/v1/admin/tours/:id', requirePermission(store, 'content:manage'), async (req, res) => { const tour = await store.archiveTour(String(req.params.id)); assert(tour, 404, 'TOUR_NOT_FOUND', 'Tour package not found'); await store.audit('admin.tour_archived', { ...clientMeta(req), userId: req.user!.id, metadata: { tourId: tour.id } }); res.json({ tour }); });

  app.post('/api/v1/admin/notifications', requirePermission(store, 'notifications:send'), async (req, res) => {
  ...
  if (config.serveStatic) { app.get(/^\/admin(?:\/.*)?$/, (_req, res) => res.sendFile(path.join(config.publicDir, 'admin.html'))); app.use(express.static(config.publicDir, { index: 'index.html', maxAge: config.isProduction ? '1h' : 0 })); app.get(/^(?!\/api\/|\/admin(?:\/|$)|\/healthz$|\/readyz$).*/, (_req, res) => res.sendFile(path.join(config.publicDir, 'index.html'))); }
  app.use(notFound);
