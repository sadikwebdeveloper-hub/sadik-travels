...
export function buildApp() {
  ...
  app.patch('/api/v1/admin/users/:id/role', requireAdmin(store), async (req, res) => { assert(['admin', 'super_admin'].includes(req.user!.role), 403, 'ADMIN_ONLY', 'Only an admin can change roles'); const input = toInput(roleRequest, req.body); const user = await store.setUserRole(String(req.params.id), input.role); assert(user, 404, 'USER_NOT_FOUND', 'User not found'); res.json({ user }); });
